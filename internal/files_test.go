...
func TestUcAccFilesUploadAndDownload(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, fileHead.ContentType, "application/octet-stream")
	assert.Equal(t, fileHead.ContentLength, int64(4))
	t.Cleanup(func() {
...
func TestUcAccFilesDelete(t *testing.T) {
	...
	assert.NoError(t, createFile(t, ctx, w, filePath, "Hello, world!"))
...
func TestUcAccFilesGetMetadata(t *testing.T) {
	...
	filePath := RandomName("/Volumes/" + volume.CatalogName + "/" + volume.SchemaName + "/" + volume.Name + "/file-")
	assert.NoError(t, createFile(t, ctx, w, filePath, "12345"))
...
func TestUcAccFilesCreateDirectory(t *testing.T) {
	...
	assert.NoError(t, createDirectory(t, ctx, w, directoryPath))
}
...
func TestUcAccFilesListDirectoryContents(t *testing.T) {
	...
	directoryPath := RandomName("/Volumes/" + volume.CatalogName + "/" + volume.SchemaName + "/" + volume.Name + "/directory-")
	assert.NoError(t, createFile(t, ctx, w, directoryPath+"/file01.txt", "Hello, world!"))
	assert.NoError(t, createFile(t, ctx, w, directoryPath+"/file02.txt", "Hello, world!"))
	assert.NoError(t, createFile(t, ctx, w, directoryPath+"/file03.txt", "Hello, world!"))
...
func TestAccDbfsOpen(t *testing.T) {
	...
	// Upload through [io.Writer] should fail because the file exists.
	{
		...
		assert.ErrorContains(t, err, "dbfs open: A file or directory already exists at the input path")
	}
