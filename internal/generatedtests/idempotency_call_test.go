	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
				// Return 404 for host metadata endpoint and prevent config resolution from interfering with test call counting.
				if request.Method == "GET" && request.URL.Path == "/.well-known/databricks-config" {
