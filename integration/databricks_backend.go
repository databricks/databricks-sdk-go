...
type Backend struct {
	lockClient *workspace.WorkspaceClient
	lockDir    string
	lockName   string
}
...
func (d *Backend) PrepareBackend(ctx context.Context, lockId string) error {
	...
	w, err := workspace.NewWorkspaceClient(c)
	if err != nil {
...
func (d *Backend) getCurrentLockState(ctx context.Context) (*core.LockState, error) {
	resp, err := d.lockClient.Download(ctx, d.getLockPath())
...
func (d *Backend) uploadLockState(ctx context.Context, contents *core.LockState, overwrite bool) error {
	...
	return d.lockClient.Upload(ctx, d.getLockPath(), bytes.NewReader(buf), opts...)
}
