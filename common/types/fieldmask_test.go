...
func TestFieldMask_MarshalJSON(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "single field",
			fieldMask: FieldMask{Fields: []string{"name"}},
			expected:  `"name"`,
		},
		{
			name:      "multiple fields",
			fieldMask: FieldMask{Fields: []string{"name", "age", "email"}},
			expected:  `"name,age,email"`,
		},
		{
			name:      "fields with spaces",
			fieldMask: FieldMask{Fields: []string{"first name", "last name"}},
			expected:  `"first name,last name"`,
		},
		{
			name:      "fields with special characters",
			fieldMask: FieldMask{Fields: []string{"user_id", "created_at", "is_active"}},
...
func TestFieldMask_UnmarshalJSON(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:    "empty string",
			input:   `""`,
			want:    FieldMask{Fields: []string{}},
			wantErr: false,
		},
		{
			name:    "single field",
			input:   `"name"`,
			want:    FieldMask{Fields: []string{"name"}},
			wantErr: false,
		},
		{
			name:    "multiple fields",
			input:   `"name,age,email"`,
			want:    FieldMask{Fields: []string{"name", "age", "email"}},
			wantErr: false,
		},
		{
			name:    "fields with spaces",
			input:   `"first name,last name"`,
			want:    FieldMask{Fields: []string{"first name", "last name"}},
			wantErr: false,
		},
		{
			name:    "fields with special characters",
			input:   `"user_id,created_at,is_active"`,
			want:    FieldMask{Fields: []string{"user_id", "created_at", "is_active"}},
			wantErr: false,
		},
		{
			name:    "trailing comma",
			input:   `"name,age,"`,
			want:    FieldMask{Fields: []string{"name", "age", ""}},
			wantErr: false,
		},
		{
			name:    "leading comma",
...
func TestFieldMask_EncodeValues(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "custom key",
			fieldMask: FieldMask{Fields: []string{"id", "status"}},
			key:       "select",
			want:      "id,status",
			wantErr:   false,
		},
