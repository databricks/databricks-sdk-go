...
type databricksConfig interface {
	Authenticate(r *http.Request) error
	EnsureResolved() error
	GetAuthType() string
	GetAccountID() string
	GetHost() string
	GetRetryTimeoutSeconds() int
	GetRateLimitPerSecond() int
	GetDebugTruncateBytes() int
	IsDebugHeaders() bool
...
func New(cfg databricksConfig) (*DatabricksClient, error) {
	err := cfg.EnsureResolved()
	if err != nil {
		return nil, err
	}
	retryTimeout := time.Duration(orDefault(cfg.GetRetryTimeoutSeconds(), 300)) * time.Second
	httpTimeout := time.Duration(orDefault(cfg.GetRetryTimeoutSeconds(), 60)) * time.Second
	rateLimiter := rate.NewLimiter(rate.Limit(orDefault(cfg.GetRateLimitPerSecond(), 15)), 1)
	debugTruncateBytes := orDefault(cfg.GetDebugTruncateBytes(), 96)
	return &DatabricksClient{
		Config:             cfg,
		debugHeaders:       cfg.IsDebugHeaders(),
...
func (c *DatabricksClient) completeUrl(r *http.Request) error {
	...
	url, err := url.Parse(c.Config.GetHost())
	if err != nil {
		return err
	}
	r.URL.Host = url.Host
...
func (c *DatabricksClient) recordRequestLog(response *http.Response,
	...
	if c.debugHeaders {
		...
		if host != "" {
			sb.WriteString("> * Host: ")
			sb.WriteString(escapeNewLines(host))
			sb.WriteString("\n")
		}
...
func (c *DatabricksClient) addAuthHeaderToUserAgent(r *http.Request) error {
	ctx := useragent.InContext(r.Context(), "auth", c.Config.GetAuthType())
