...
// CustomerManagedKeyAwsKeyInfoToCustomerManagedKeyIdMap calls [EncryptionKeysAPI.List] and creates a map of results with [CustomerManagedKey].AwsKeyInfo as key and [CustomerManagedKey].CustomerManagedKeyId as value.
//
// Returns an error if there's more than one [CustomerManagedKey] with the same .AwsKeyInfo.
//
// Note: All [CustomerManagedKey] instances are loaded into memory before creating a map.
//
// This method is generated by Databricks SDK Code Generator.
func (a *EncryptionKeysAPI) CustomerManagedKeyAwsKeyInfoToCustomerManagedKeyIdMap(ctx context.Context) (map[string]string, error) {
	ctx = useragent.InContext(ctx, "sdk-feature", "name-to-id")
	mapping := map[string]string{}
	result, err := a.List(ctx)
	if err != nil {
	...
	for _, v := range result {
		...
		_, duplicate := mapping[key]
		if duplicate {
			return nil, fmt.Errorf("duplicate .AwsKeyInfo: %s", key)
		}
...
// GetByAwsKeyInfo calls [EncryptionKeysAPI.CustomerManagedKeyAwsKeyInfoToCustomerManagedKeyIdMap] and returns a single [CustomerManagedKey].
//
// Returns an error if there's more than one [CustomerManagedKey] with the same .AwsKeyInfo.
//
...
//
// This method is generated by Databricks SDK Code Generator.
func (a *EncryptionKeysAPI) GetByAwsKeyInfo(ctx context.Context, name string) (*CustomerManagedKey, error) {
	ctx = useragent.InContext(ctx, "sdk-feature", "get-by-name")
	result, err := a.List(ctx)
	if err != nil {
		return nil, err
