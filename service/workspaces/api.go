...
type WorkspacesService interface {
    ...
    // Given a workspace specified by ID, this request gets a list of all
    // associations with key configuration objects that encapsulate
    // customer-managed keys that encrypt managed services, workspace storage,
    // or in some cases both. **Important**: In the current implementation, keys
    // cannot be rotated or removed from a workspace. It is possible for a
    // workspace to show a storage customer-managed key having been attached and
    // then detached if the workspace was updated to use the key and the update
    // operation failed. **Important**: Customer-managed keys are supported only
    // for some deployment types and subscription types. This operation is
    // available only if your account is on the E2 version of the platform.
    GetWorkspaceKeyHistory(ctx context.Context, getWorkspaceKeyHistoryRequest GetWorkspaceKeyHistoryRequest) (*ListWorkspaceEncryptionKeyRecordsResponse, error)
	...
	GetWorkspaceKeyHistoryByAccountIdAndWorkspaceId(ctx context.Context, accountId string, workspaceId int64) (*ListWorkspaceEncryptionKeyRecordsResponse, error)
}

func New(client *client.DatabricksClient) WorkspacesService {
...
// same or different. **Important**: This operation is asynchronous. A response
// with HTTP status code 200 means the request has been accepted and is in
// progress, but does not mean that the workspace deployed successfully and is
// running. The initial workspace status is typically `PROVISIONING`. Use the
// workspace ID (`workspace_id`) field in the response to identify the new
// workspace and make repeated `GET` requests with the workspace ID and check
// its status. The workspace becomes available when the status changes to
// `RUNNING`. You can share one customer-managed VPC with multiple workspaces in
...
// If you plan to share one VPC with multiple workspaces, be sure to size your
// VPC and subnets accordingly. Because a Databricks Account API network
// configuration encapsulates this information, you cannot reuse a Databricks
// Account API network configuration across workspaces. For detailed
...
func (a *WorkspacesAPI) CreateWorkspace(ctx context.Context, request CreateWorkspaceRequest) (*Workspace, error) {
	...
	path := fmt.Sprintf("/accounts/%v/workspaces", request.AccountId)
...
func (a *WorkspacesAPI) DeleteWorkspace(ctx context.Context, request DeleteWorkspaceRequest) error {
	path := fmt.Sprintf("/accounts/%v/workspaces/%v", request.AccountId, request.WorkspaceId)
...
// platform or on a select custom plan that allows multiple workspaces per
// account.
func (a *WorkspacesAPI) GetAllWorkspaces(ctx context.Context, request GetAllWorkspacesRequest) (*ListWorkspacesResponse, error) {
	var listWorkspacesResponse ListWorkspacesResponse
	path := fmt.Sprintf("/accounts/%v/workspaces", request.AccountId)
	err := a.client.Get(ctx, path, request, &listWorkspacesResponse)
	return &listWorkspacesResponse, err
}

// Get information including status for a Databricks workspace, specified by ID.
...
// available when the status changes to `RUNNING`. For detailed instructions of
...
// Given a workspace specified by ID, this request gets a list of all
// associations with key configuration objects that encapsulate customer-managed
...
func (a *WorkspacesAPI) GetWorkspaceKeyHistory(ctx context.Context, request GetWorkspaceKeyHistoryRequest) (*ListWorkspaceEncryptionKeyRecordsResponse, error) {
	...
	path := fmt.Sprintf("/accounts/%v/workspaces/%v/customer-managed-key-history", request.AccountId, request.WorkspaceId)
...
// The `PATCH` operation on this endpoint can update a workspace configuration
// for either a running workspace or a failed workspace. The elements that can
// be updated varies between these two use cases. ### Update a failed workspace
// You can update a Databricks workspace configuration for failed workspace
// deployment for some but not all fields. This request supports updating only
// the following fields of a failed workspace: - Credential configuration ID -
// Storage configuration ID - Network configuration ID. Used only if you use
// customer-managed VPC. - Key configuration ID for managed services (control
// plane storage, such as notebook source and Databricks SQL queries). Used only
// if you use customer-managed keys for managed services. - Key configuration ID
// for workspace storage (root S3 bucket and optionally EBS volumes). Used only
// if you use customer-managed keys for workspace storage. IMPORTANT: If the
...
// storage. After calling the `PATCH` operation to update the workspace
// configuration, make repeated `GET` requests with the workspace ID and check
// the workspace status. The workspace is successful if the status changes to
...
// customer-managed VPC. Note: You cannot use a network configuration update in
// this API to add support for PrivateLink (in Public Preview). To add
// PrivateLink to an existing workspace, contact your Databricks representative.
...
// configuration for workspace storage. **Important**: For updating running
// workspaces, this API is unavailable on Mondays, Tuesdays, and Thursdays from
// 4:30pm-7:30pm PST due to routine maintenance. Plan your workspace updates
// accordingly. For questions about this schedule, contact your Databricks
// representative. **Important**: To update a running workspace, your workspace
// must have no running cluster instances, which includes all-purpose clusters,
// job clusters, and pools that may have running clusters. Terminate all cluster
// instances in the workspace before calling this API. ### Wait until changes
// take effect After calling the `PATCH` operation to update the workspace
// configuration, make repeated `GET` requests with the workspace ID and check
// the workspace status and the status of the fields. * For workspaces with a
// Databricks-managed VPC, the workspace status becomes `PROVISIONING`
// temporarily (typically under 20 minutes). If the workspace update is
// successful, the workspace status changes to `RUNNING`. Note that you can also
// check the workspace status in the [Account
// Console](https://docs.databricks.com/administration-guide/account-settings-e2/account-console-e2.html).
...
// other unexpected behavior. * For workspaces with a customer-managed VPC, the
...
// may take a few minutes to update, so continue to check the workspace until
// you observe it has updated. If the update fails, the workspace may revert
// silently to its original configuration. Once the workspace has updated, you
...
// successfully, fail, or could cause other unexpected behavior. If you update
// the _storage_ customer-managed key configurations, it takes 20 minutes for
// the changes to fully take effect. During the 20 minute wait, it is important
...
// about availability, contact your Databricks representative. This operation is
...
func (a *WorkspacesAPI) GetAllWorkspacesByAccountId(ctx context.Context, accountId string) (*ListWorkspacesResponse, error) {
