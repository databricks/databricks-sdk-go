...
type SecretsService interface {
	...
	// exists. Throws ``RESOURCE_LIMIT_EXCEEDED`` if maximum number of scopes
	// in the workspace is exceeded. Throws ``INVALID_PARAMETER_VALUE`` if the
	...
	// API. Example response: .. code:: { &#34;principal&#34;: &#34;data-scientists&#34;,
	...
	// &#34;last_updated_timestamp&#34;: &#34;1520467595000&#34; }, ] } The
	...
	// permission. Users must have the ``MANAGE`` permission to invoke this
	// API. Example request: .. code:: { &#34;scope&#34;: &#34;my-secret-scope&#34;,
	// &#34;principal&#34;: &#34;data-scientists&#34;, &#34;permission&#34;: &#34;READ&#34; } The principal is
	// a user or group name corresponding to an existing Databricks principal
	// to be granted or revoked access. Throws ``RESOURCE_DOES_NOT_EXIST`` if
	// no such secret scope exists. Throws ``RESOURCE_ALREADY_EXISTS`` if a
	...
	// or ``MANAGE`` permission on the secret scope. The secret key must
	// consist of alphanumeric characters, dashes, underscores, and periods,
	// and cannot exceed 128 characters. The maximum allowed secret value size
	// is 128 KB. The maximum number of secrets in a given scope is 1000.
	// Example request: .. code:: { &#34;scope&#34;: &#34;my-databricks-scope&#34;, &#34;key&#34;:
	// &#34;my-string-key&#34;, &#34;string_value&#34;: &#34;foobar&#34; } The input fields
	// &#34;string_value&#34; or &#34;bytes_value&#34; specify the type of the secret, which
	// will determine the value returned when the secret value is requested.
	// Exactly one must be specified. Throws ``RESOURCE_DOES_NOT_EXIST`` if no
	// such secret scope exists. Throws ``RESOURCE_LIMIT_EXCEEDED`` if maximum
	// number of secrets in scope is exceeded. Throws
	// ``INVALID_PARAMETER_VALUE`` if the key name or value length is invalid.
	// Throws ``PERMISSION_DENIED`` if the user does not have permission to
	// make this API call.
	PutSecret(ctx context.Context, putSecretRequest PutSecretRequest) error
}

func New(client *client.DatabricksClient) SecretsService {
	return &SecretsAPI{
		client: client,
	}
}
...
// principal for this option is the group ``users``, which contains all users
// in the workspace. If ``initial_manage_principal`` is not specified, the
// initial ACL with ``MANAGE`` permission applied to the scope is assigned to
// the API request issuer&#39;s user identity. If ``scope_backend_type`` is
// ``azure_keyvault``, a secret scope is created with secrets from a given
// Azure KeyVault. The caller must provide the keyvault_resource_id and the
// tenant_id for the key vault. If ``scope_backend_type`` is ``databricks`` or
// is unspecified, an empty secret scope is created and stored in Databricks&#39;s
// own storage. Throws ``RESOURCE_ALREADY_EXISTS`` if a scope with the given
// name already exists. Throws ``RESOURCE_LIMIT_EXCEEDED`` if maximum number of
...
// as follows: * ``MANAGE`` - Allowed to change ACLs, and read and write to
// this secret scope. * ``WRITE`` - Allowed to read and write to this secret
// scope. * ``READ`` - Allowed to read this secret scope and list what secrets
// are available. Note that in general, secret values can only be read from
// within a command on a cluster (for example, through a notebook). There is no
// API to read the actual secret value material outside of a cluster. However,
// the user&#39;s permission will be applied based on who is executing the command,
// and they must have at least READ permission. Users must have the ``MANAGE``
// permission to invoke this API. Example request: .. code:: { &#34;scope&#34;:
// &#34;my-secret-scope&#34;, &#34;principal&#34;: &#34;data-scientists&#34;, &#34;permission&#34;: &#34;READ&#34; }
// The principal is a user or group name corresponding to an existing
// Databricks principal to be granted or revoked access. Throws
// ``RESOURCE_DOES_NOT_EXIST`` if no such secret scope exists. Throws
