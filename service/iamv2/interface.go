...
type AccountIamV2Service interface {
	...
	// Syncs a group with the given external ID from the customer's IdP. If the
	// group does not exist, it will be created in the account. If the customer
	// is not onboarded onto Automatic Identity Management (AIM), this will
	// return an error. Synced information is cached for 30 minutes, so
	// subsequent calls to this method will not result in a full sync unless the
	// cache is stale. If this is triggered while the cache is still valid, it
	// will return the cached group information.
	SyncGroup(ctx context.Context, request SyncGroupRequest) (*SyncGroupResponse, error)

	// Syncs a sp with the given external ID from the customer's IdP. If the sp
	// does not exist, it will be created. If the customer is not onboarded onto
	// Automatic Identity Management (AIM), this will return an error. Synced
	// information is cached for 30 minutes, so subsequent calls to this method
	// will not result in a full sync unless the cache is stale. If this is
	// triggered while the cache is still valid, it will return the cached SP
	// information.
	SyncServicePrincipal(ctx context.Context, request SyncServicePrincipalRequest) (*SyncServicePrincipalResponse, error)

	// Syncs a user with the given external ID from the customer's IdP. If the
	...
	// Synced information is cached for 30 minutes, so subsequent calls to this
	// method will not result in a full sync unless the cache is stale. If this
	// is triggered while the cache is still valid, it will return the cached
	// user information.
	SyncUser(ctx context.Context, request SyncUserRequest) (*SyncUserResponse, error)
...
type WorkspaceIamV2Service interface {
	...
	// cache is stale. If this is triggered while the cache is still valid, it
	// will return the cached group information.
	SyncGroupProxy(ctx context.Context, request SyncGroupProxyRequest) (*SyncGroupResponse, error)
	...
	// triggered while the cache is still valid, it will return the cached SP
	// information.
	SyncServicePrincipalProxy(ctx context.Context, request SyncServicePrincipalProxyRequest) (*SyncServicePrincipalResponse, error)
	...
	// is triggered while the cache is still valid, it will return the cached
	// user information.
	SyncUserProxy(ctx context.Context, request SyncUserProxyRequest) (*SyncUserResponse, error)
