...
const ConnectionTypeJdbc ConnectionType = `JDBC`
...
const ConnectionTypeZendesk ConnectionType = `ZENDESK`
...
type ListMcpServicesRequest struct {
	...
	// `BASIC` when unset or `VIEW_UNSPECIFIED`.
...
type ListModelProviderServicesRequest struct {
	...
	// individually.
	Parent string `json:"-" url:"parent,omitempty"`
	// Fields to return for each service. `FULL` includes inference-table
	// details and rate-limit principal names. `BASIC` omits inference-table
	// details and omits principal names from rate limits. Defaults to `BASIC`
	// when unset or `VIEW_UNSPECIFIED`.
...
type McpServiceConfig struct {
	...
	// rate limits are not supported for MCP services. Empty when no rate limit
	// is configured.
...
type ModelProviderService struct {
	...
	// limits, and payload logging. Required on Create. On Update, it is
...
type ModelProviderServiceConfig struct {
	...
	// entire provider service, not per request. Upstream authentication is
	// configured separately in `provider`.
...
type ModelProviderServiceConfigAzureOpenAiProviderDirectConfig struct {
	...
	// value is not carried here. On read, the resolved `id` and `is_deleted`
	// are also populated. Only supported on Azure-hosted workspaces; Create
...
type ModelProviderServiceConfigServiceCredential struct {
	...
	// `credentials/{name}`. On Create, supply the name here. On read, this
	// field reflects the credential's current name.
	Name string `json:"name"`
}
...
type PolicyInfo struct {
	...
	// a whole.
	Grant *GrantOptions `json:"grant,omitempty"`
...
type RateLimit struct {
	...
	// principal, the service as a whole, each user by default, or a request
	// tag.
...
const SecurableKindTableDeltaUniformIcebergExternalDeltasharing SecurableKind = `TABLE_DELTA_UNIFORM_ICEBERG_EXTERNAL_DELTASHARING`
...
func (f *SecurableKind) Values() []SecurableKind {
	return []SecurableKind{
		...
		SecurableKindTableDeltaUniformIcebergExternalDeltasharing,
...
type UpdateModelProviderServiceRequest struct {
	...
	// replacement must include every required field; any optional field you
	// omit is cleared. To preserve sibling fields, use one or more granular
	// paths: `comment`, `config.provider`, `config.allow_all_targets`,
	// `config.targets`, `config.forward_headers`,
...
type UpdateModelServiceRequest struct {
	...
	// `config.routing.routing_strategy`,
