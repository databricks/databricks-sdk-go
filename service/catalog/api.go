...
func (a *TablesAPI) ListAll(ctx context.Context, request ListTablesRequest) ([]TableInfo, error) {
	...
	listing := ListTablesInternal{
		CatalogName:          request.CatalogName,
		IncludeDeltaMetadata: request.IncludeDeltaMetadata,
...
func (a *TablesAPI) ListSummariesAll(ctx context.Context, request ListSummariesRequest) ([]TableSummary, error) {
	...
	listing := ListSummariesInternal{
		CatalogName:       request.CatalogName,
		MaxResults:        request.MaxResults,
		SchemaNamePattern: request.SchemaNamePattern,
		TableNamePattern:  request.TableNamePattern,
	}
	ctx = useragent.InContext(ctx, "sdk-feature", "pagination")
	for {
		response, err := a.impl.ListSummaries(ctx, listing)
		if err != nil {
			return nil, err
		}
		if len(response.Tables) == 0 {
