...
type CredentialsInterface interface {
	...
	// **external_location_name** and **url** need to be provided. If only one of
	// them is provided, it will be used for validation. And if both are provided,
	// the **url** will be used for validation, and **external_location_name** will
	// be ignored when checking overlapping urls. Either the **credential_name** or
...
type FunctionsInterface interface {
	...
	//   - Have the **USE_CATALOG** privilege on the function's parent catalog, the
	//     **USE_SCHEMA** privilege on the function's parent schema, and the
	...
	// be a member of the group that is the new function owner.
	//
	//   - Is a metastore admin
	//   - Is the owner of the function's parent catalog
	//   - Is the owner of the function's parent schema and has the **USE_CATALOG**
	//     privilege on its parent catalog
	//   - Is the owner of the function itself and has the **USE_CATALOG** privilege
	//     on its parent catalog as well as the **USE_SCHEMA** privilege on the
...
type MetastoresInterface interface {
	...
	// **workspace_id** exists, it will be overwritten by the new **metastore_id**
	// and **default_catalog_name**. The caller must be an account admin.
	...
	// path. By default (if the **owner** field is not set), the owner of the new
	// metastore is the user calling the **createMetastore** API. If the **owner**
	...
	// admin. If the **owner** field is set to the empty string (**""**), the
...
type QualityMonitorsInterface interface {
	...
	//  2. have **USE_CATALOG** on the table's parent catalog, be an owner of the
	//     table's parent schema, and have **SELECT** access on the table.
	...
	//  2. have **USE_CATALOG** on the table's parent catalog and be an owner of the
	//     table's parent schema
	//  3. have the following permissions:
	//
	//   - **USE_CATALOG** on the table's parent catalog
	//   - **USE_SCHEMA** on the table's parent schema
...
type SchemasInterface interface {
	...
	// a metastore admin. If the caller is a metastore admin, only the **owner**
	// field can be changed in the update. If the **name** field must be updated,
...
type StorageCredentialsInterface interface {
	...
	// Validates a storage credential. At least one of **external_location_name**
	// and **url** need to be provided. If only one of them is provided, it will be
	// used for validation. And if both are provided, the **url** will be used for
	// validation, and **external_location_name** will be ignored when checking
	// overlapping urls.
	//
	// Either the **storage_credential_name** or the cloud-specific credential must
	// be provided.
	//
...
type TableConstraintsInterface interface {
	...
	// these conditions:
	//
	//   - the user must have the **USE_CATALOG** privilege on the table's parent
	//     catalog, the **USE_SCHEMA** privilege on the table's parent schema, and
	//     be the owner of the table.
	//   - if the new constraint is a **ForeignKeyConstraint**, the user must have
	//     the **USE_CATALOG** privilege on the referenced parent table's catalog,
	//     the **USE_SCHEMA** privilege on the referenced parent table's schema, and
	//     be the owner of the referenced parent table.
	...
	//     permissions on all of the child tables: the **USE_CATALOG** privilege on
	//     the table's catalog, the **USE_SCHEMA** privilege on the table's schema,
...
type TablesInterface interface {
	...
	//   - Be the owner of the parent schema and have the **USE_CATALOG** privilege
	//     on the parent catalog
...
type VolumesInterface interface {
	...
	//
	//   - The caller must have **CREATE EXTERNAL VOLUME** privilege on the external
	//     location.
