...
// WithImpl could be used to override low-level API implementations for unit
// testing purposes with [github.com/golang/mock] or other mocking frameworks.
func (a *ArtifactAllowlistsAPI) WithImpl(impl ArtifactAllowlistsService) *ArtifactAllowlistsAPI {
	a.impl = impl
	return a
}
...
func NewSecurableTags(client *client.DatabricksClient) *SecurableTagsAPI {
	return &SecurableTagsAPI{
		impl: &securableTagsImpl{
			client: client,
		},
	}
}
...
type SecurableTagsAPI struct {
	// impl contains low-level REST API interface, that could be overridden
	// through WithImpl(SecurableTagsService)
	impl SecurableTagsService
}

// WithImpl could be used to override low-level API implementations for unit
// testing purposes with [github.com/golang/mock] or other mocking frameworks.
func (a *SecurableTagsAPI) WithImpl(impl SecurableTagsService) *SecurableTagsAPI {
	a.impl = impl
	return a
}

// Impl returns low-level SecurableTags API implementation
func (a *SecurableTagsAPI) Impl() SecurableTagsService {
	return a.impl
}

// Get tags for a securable.
//
// Gets tag assignments for an entity. The caller must be either the owner of
// the securable, or a metastore admin, or have at least USE / SELECT privilege
// on the associated securable.
//
// This method is generated by Databricks SDK Code Generator.
func (a *SecurableTagsAPI) ListAll(ctx context.Context, request ListSecurableTagsRequest) ([]TagSecurableAssignment, error) {
	response, err := a.impl.List(ctx, request)
	if err != nil {
		return nil, err
	}
	return response.TagAssignments, nil
}
...
// the securable, or a metastore admin, or have at least USE / SELECT privilege
// on the associated securable.
func (a *SecurableTagsAPI) ListBySecurableTypeAndFullName(ctx context.Context, securableType ListSecurableType, fullName string) (*TagSecurableAssignmentsList, error) {
	return a.impl.List(ctx, ListSecurableTagsRequest{
		SecurableType: securableType,
		FullName:      fullName,
	})
}
...
//
// Update tag assignments for an entity The caller must be either the owner of
// the securable, or a metastore admin, or have at least USE / SELECT and
// APPLY_TAG privilege on the associated securable.
func (a *SecurableTagsAPI) Update(ctx context.Context, request UpdateTags) (*TagSecurableAssignmentsList, error) {
	return a.impl.Update(ctx, request)
}
...
func NewSubentityTags(client *client.DatabricksClient) *SubentityTagsAPI {
	return &SubentityTagsAPI{
		impl: &subentityTagsImpl{
			client: client,
		},
	}
}
...
type SubentityTagsAPI struct {
	// impl contains low-level REST API interface, that could be overridden
	// through WithImpl(SubentityTagsService)
	impl SubentityTagsService
}

// WithImpl could be used to override low-level API implementations for unit
// testing purposes with [github.com/golang/mock] or other mocking frameworks.
func (a *SubentityTagsAPI) WithImpl(impl SubentityTagsService) *SubentityTagsAPI {
	a.impl = impl
	return a
}

// Impl returns low-level SubentityTags API implementation
func (a *SubentityTagsAPI) Impl() SubentityTagsService {
	return a.impl
}

// Get tags for a subentity.
//
// Gets tag assignments for a subentity associated with a securable entity. Eg.
// column of a table The caller must be either the owner of the securable, or a
// metastore admin, or have at least USE / SELECT privilege on the associated
// securable.
//
// This method is generated by Databricks SDK Code Generator.
func (a *SubentityTagsAPI) ListAll(ctx context.Context, request ListSubentityTagsRequest) ([]TagsSubentityAssignment, error) {
	response, err := a.impl.List(ctx, request)
	if err != nil {
		return nil, err
	}
...
// metastore admin, or have at least USE / SELECT privilege on the associated
// securable.
func (a *SubentityTagsAPI) ListBySecurableTypeAndFullNameAndSubentityName(ctx context.Context, securableType ListSecurableType, fullName string, subentityName string) (*TagSubentityAssignmentsList, error) {
	return a.impl.List(ctx, ListSubentityTagsRequest{
		SecurableType: securableType,
		FullName:      fullName,
...
//
// Update tag assignments for a subentity associated with a securable entity.
// The caller must be either the owner of the securable, or a metastore admin,
// or have at least USE / SELECT and APPLY_TAG privilege on the associated
// securable.
func (a *SubentityTagsAPI) Update(ctx context.Context, request UpdateTags) (*TagSubentityAssignmentsList, error) {
	return a.impl.Update(ctx, request)
}
