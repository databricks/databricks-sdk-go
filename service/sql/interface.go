...
// the presence of the `next_chunk_internal_link` field, and iteratively `GET`
...
//
// When result set in INLINE mode becomes larger, it will transfer results in
// chunks, each up to 4 MiB. After receiving the initial chunk with
...
// chunk, when there are additional chunks remaining; it can be found in the
...
// :method:statementexecution/getStatement will return a 404 NOT FOUND error. -
// In practice, this means that a CANCEL and subsequent poll will often return a
// NOT FOUND.
//
...
type StatementExecutionService interface {
	...
	//
	// **NOTE** This call currently may take up to 5 seconds to get latest
	// status and result.
