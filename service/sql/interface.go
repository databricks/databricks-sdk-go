...
type StatementExecutionService interface {
	...
	// field, and iteratively :method:StatementExecution/getStatement those
	// paths until that field is unset: `GET
	...
	// seconds.
	//
	// When result set in INLINE mode becomes larger, it will be transfer
	// results in chunks, each up to 4 MiB. After receiving the initial chunk
	// get :method:StatementExecution/executeStatement or
	...
	// next chunk, when there are additiona chunks remaining; it can be found in
