...
// PolicyFamilyNameToPolicyFamilyIdMap calls [PolicyFamiliesAPI.ListAll] and creates a map of results with [PolicyFamily].Name as key and [PolicyFamily].PolicyFamilyId as value.
//
// Returns an error if there's more than one [PolicyFamily] with the same .Name.
//
// Note: All [PolicyFamily] instances are loaded into memory before creating a map.
//
// This method is generated by Databricks SDK Code Generator.
func (a *PolicyFamiliesAPI) PolicyFamilyNameToPolicyFamilyIdMap(ctx context.Context, request ListPolicyFamiliesRequest) (map[string]string, error) {
	ctx = useragent.InContext(ctx, "sdk-feature", "name-to-id")
	mapping := map[string]string{}
	result, err := a.ListAll(ctx, request)
	if err != nil {
	...
	for _, v := range result {
		...
		mapping[key] = v.PolicyFamilyId
	}
	return mapping, nil
}

// GetByName calls [PolicyFamiliesAPI.PolicyFamilyNameToPolicyFamilyIdMap] and returns a single [PolicyFamily].
//
// Returns an error if there's more than one [PolicyFamily] with the same .Name.
//
// Note: All [PolicyFamily] instances are loaded into memory before returning matching by name.
//
// This method is generated by Databricks SDK Code Generator.
func (a *PolicyFamiliesAPI) GetByName(ctx context.Context, name string) (*PolicyFamily, error) {
	ctx = useragent.InContext(ctx, "sdk-feature", "get-by-name")
	result, err := a.ListAll(ctx, ListPolicyFamiliesRequest{})
	if err != nil {
		return nil, err
	}
	tmp := map[string][]PolicyFamily{}
	for _, v := range result {
		key := v.Name
		tmp[key] = append(tmp[key], v)
	}
	alternatives, ok := tmp[name]
	if !ok || len(alternatives) == 0 {
		return nil, fmt.Errorf("PolicyFamily named '%s' does not exist", name)
	}
	if len(alternatives) > 1 {
		return nil, fmt.Errorf("there are %d instances of PolicyFamily named '%s'", len(alternatives), name)
	}
	return &alternatives[0], nil
}
