...
type ExecuteCommandSyncRequest struct {
	...
	// Executable or command to run (e.g. `/bin/echo`, `python3`). A request
	// with no `cmd` is rejected with `INVALID_ARGUMENT`. Not audited (no
	// `compliance.audit_mode`): the command can carry secrets, and as a
	// data-plane service lakebox must not record privileged customer content in
	// its audit log.
	Cmd string `json:"cmd"`
...
type ExecuteCommandSyncResponse struct {
	...
	// with the Unicode replacement character rather than failing the request;
	// use the streaming `ExecuteCommand` RPC for byte-exact output.
	...
	// for commands that can produce large output, use the streaming
	// `ExecuteCommand` RPC, which is not subject to this cap.
