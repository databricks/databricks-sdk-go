...
type SandboxService interface {
	...
	// captured stdout, stderr and exit code in a single response. Unary
	// convenience variant of the streaming command-execution API for callers
	// that only need a command's final result (e.g. `curl`, the SDK's
	// `sandbox.exec`). The streaming `ExecuteCommand` RPC remains for
	// interactive and long-running use.
