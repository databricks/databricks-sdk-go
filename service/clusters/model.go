...
type AwsAttributes struct {
    ...
    // Legacy node types cannot specify custom EBS volumes. For node types with 
    // no instance store, at least one EBS volume needs to be specified; 
    // otherwise, cluster creation will fail. These EBS volumes will be mounted 
    // at ``/ebs0``, ``/ebs1``, and etc. Instance store volumes will be mounted 
    // at ``/local_disk0``, ``/local_disk1``, and etc. If EBS volumes are 
    // attached, Databricks will configure Spark to use only the EBS volumes 
    // for scratch storage because heterogenously sized scratch devices can 
    // lead to inefficient disk utilization. If no EBS volumes are attached, 
    // Databricks will configure Spark to use instance store volumes. Please 
    // note that if EBS volumes are specified, then the Spark configuration 
...
type AzureAttributes struct {
    ...
    // the cluster driver node is placed on an on-demand instance. If this 
    // value is greater than or equal to the current cluster size, all nodes 
    // will be placed on on-demand instances. If this value is less than the 
    // current cluster size, ``first_on_demand`` nodes will be placed on 
    // on-demand instances and the remainder will be placed on ``availability`` 
    ...
    // not specified, the default value is -1, which specifies that the 
    // instance cannot be evicted on the basis of price, and only on the basis 
...
type ClusterAttributes struct {
    ...
    // :ref:`clusterClusterServicelistSparkVersions` API call. This is the 
    // Spark version provided from the user input (API/UI) and may be different 
    // from the effective_spark_version, which is the spark version that is 
    // actually run in the dataplane. See index 45 for more context. 
...
type EventDetails struct {
    ...
    // termination. * On a RESIZE_COMPLETE event, this indicates the reason 
...
type GcpAttributes struct {
    ...
    // run on preemptible VMs, on-demand VMs, or preemptible VMs with a 
...
type SparkNode struct {
    ...
    // timestamp when the container is placed on the ResourceManager, before 
    // its launch and setup by the NodeDaemon. This timestamp is the same as 
    // the creation timestamp in the database. 
...
type SparkVersion struct {
    // Spark version key, for example &#34;2.1.x-scala2.11&#34;. This is the value 
    // which should be provided as the &#34;spark_version&#34; when creating a new 
    // cluster. Note that the exact Spark version may change over time for a 
    // &#34;wildcard&#34; version (i.e., &#34;2.1.x-scala2.11&#34; is a &#34;wildcard&#34; version) 
