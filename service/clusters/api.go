...
type ClustersService interface {
    ...
    // this method returns, the cluster will be in a ``PENDING`` state. The 
    // cluster will be usable once it enters a ``RUNNING`` state. Note: 
    // Databricks may not be able to acquire some of the requested nodes, due 
    // to cloud provider limitations (account limits, spot price, ...) or 
    // transient network issues. If Databricks acquires at least 85% of the 
    // requested on-demand nodes, cluster creation will succeed. Otherwise the 
    // cluster will terminate with an informative error message. An example 
    // request: .. code:: { &#34;cluster_name&#34;: &#34;my-cluster&#34;, &#34;spark_version&#34;: 
    ...
    // in a ``TERMINATED`` state. If the cluster is already in a 
    // ``TERMINATING`` or ``TERMINATED`` state, nothing will happen. An example 
    ...
    // and size. A cluster can be edited if it is in a ``RUNNING`` or 
    // ``TERMINATED`` state. If a cluster is edited while in a ``RUNNING`` 
    // state, it will be restarted so that the new attributes can take effect. 
    // If a cluster is edited while in a ``TERMINATED`` state, it will remain 
    // ``TERMINATED``. The next time it is started using the ``clusters/start`` 
    // API, the new attributes will take effect. An attempt to edit a cluster 
    // in any other state will be rejected with an ``INVALID_STATE`` error 
    // code. Clusters created by the Databricks Jobs service cannot be edited. 
    // An example request: .. code:: { &#34;cluster_id&#34;: &#34;1202-211320-brick1&#34;, 
    // &#34;num_workers&#34;: 10, &#34;spark_version&#34;: &#34;3.3.x-scala2.11&#34;, &#34;node_type_id&#34;: 
    // &#34;i3.2xlarge&#34; } 
    ...
    // &#34;timestamp&#34;: 1509505807923, &#34;event_type&#34;: &#34;TERMINATING&#34;, 
    // &#34;event_details&#34;: { &#34;termination_reason&#34;: { &#34;code&#34;: &#34;USER_REQUEST&#34;, 
    // &#34;parameters&#34;: [ &#34;username&#34;: &#34;admin&#34; ] } } ], &#34;next_page&#34;: { 
    // &#34;cluster_id&#34;: &#34;1202-211320-brick1&#34;, &#34;end_time&#34;: 1509572145487, &#34;order&#34;: 
    // &#34;DESC&#34;, &#34;offset&#34;: 50 }, &#34;total_count&#34;: 303 } Example request to retrieve 
    ...
    // clusters in the past 7 days. For example, if there is 1 pinned cluster, 
    // 4 active clusters, 45 terminated interactive clusters in the past 7 
    // days, and 50 terminated job clusters in the past 7 days, then this API 
    // returns the 1 pinned cluster, 4 active clusters, all 45 terminated 
    // interactive clusters, and the 30 most recently terminated job clusters. 
    ...
    // no effect. This API can only be called by workspace admins. An example 
    ...
    // Restarts a Spark cluster given its id. If the cluster is not currently 
    // in a ``RUNNING`` state, nothing will happen. An example request: .. 
...
// will succeed. Otherwise the cluster will terminate with an informative error 
...
// while in a ``TERMINATED`` state, it will remain ``TERMINATED``. The next 
// time it is started using the ``clusters/start`` API, the new attributes will 
// take effect. An attempt to edit a cluster in any other state will be 
// rejected with an ``INVALID_STATE`` error code. Clusters created by the 
// Databricks Jobs service cannot be edited. An example request: .. code:: { 
// &#34;cluster_id&#34;: &#34;1202-211320-brick1&#34;, &#34;num_workers&#34;: 10, &#34;spark_version&#34;: 
// &#34;3.3.x-scala2.11&#34;, &#34;node_type_id&#34;: &#34;i3.2xlarge&#34; } 
...
// &#34;events&#34;: [ { &#34;cluster_id&#34;: &#34;1202-211320-brick1&#34;, &#34;timestamp&#34;: 
...
// &#34;termination_reason&#34;: { &#34;code&#34;: &#34;USER_REQUEST&#34;, &#34;parameters&#34;: [ &#34;username&#34;: 
// &#34;admin&#34; ] } } ], &#34;next_page&#34;: { &#34;cluster_id&#34;: &#34;1202-211320-brick1&#34;, 
// &#34;end_time&#34;: 1509572145487, &#34;order&#34;: &#34;DESC&#34;, &#34;offset&#34;: 50 }, &#34;total_count&#34;: 
// 303 } Example request to retrieve the next page of events 
// ``/clusters/events?cluster_id=1202-211320-brick1&amp;end_time=1509572145487&amp;order=DESC&amp;offset=50`` 
...
// days, and up to 30 of the most recently terminated job clusters in the past 
// 7 days. For example, if there is 1 pinned cluster, 4 active clusters, 45 
...
// Permanently deletes a Spark cluster. This cluster is terminated and 
// resources are asynchronously removed. In addition, users will no longer see 
// permanently deleted clusters in the cluster list, and API users can no 
...
// ListClusters API. Unpinning a cluster that is not pinned will have no 
// effect. This API can only be called by workspace admins. An example request: 
