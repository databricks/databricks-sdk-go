...
type DbfsService interface {
	...
	// ``RESOURCE_DOES_NOT_EXIST``. If the block of data exceeds 1 MB, this
	// call will throw an exception with ``MAX_BLOCK_SIZE_EXCEEDED``. Example
	// of request: .. code:: { &#34;data&#34;: &#34;ZGF0YWJyaWNrcwo=&#34;, &#34;handle&#34;: 7904256 }
	...
	// call will throw an exception with ``RESOURCE_ALREADY_EXISTS``. A typical
	...
	// Lists the contents of a directory, or details of the file. If the file
	// or directory does not exist, this call will throw an exception with
	...
	// ``localsrc`` is the path to a local file to upload and this usage is
	// only supported with multipart form post (i.e. using -F or --form with
	// curl). Alternatively you can pass contents as base64 string. Examples:
	// .. code:: curl -u USER:PASS -F contents=&#34;BASE64&#34; -F path=&#34;PATH&#34;
	...
	// can be passed using contents (i.e. not streaming) parameter is limited
	// to 1 MB, ``MAX_BLOCK_SIZE_EXCEEDED`` will be thrown if exceeded. Please
	// use streaming upload if you want to upload large files, see
...
func New(client *client.DatabricksClient) DbfsService {
	return &DbfsAPI{
		client: client,
	}
}
...
// upload would be: 1) Issue a ``create`` call and get a handle. 2) Issue one
// or more ``add-block`` calls with the handle you have. 3) Issue a ``close``
// call with the handle you have.
...
// path=&#34;PATH&#34; https://XX.cloud.databricks.com/api/2.0/dbfs/put Please note
// that ``localsrc`` is the path to a local file to upload and this usage is
...
// be passed using contents (i.e. not streaming) parameter is limited to 1 MB,
