...
type DbfsInterface interface {
	...
	// cluster, using the File system utility (dbutils.fs). `dbutils.fs` covers the
	// functional scope of the DBFS REST API, but from notebooks. Running such
	// operations using notebooks provides better control and manageability, such as
	// selective deletes, and the possibility to automate periodic delete jobs.
	...
	// **contents** parameter is limited to 1 MB. `MAX_BLOCK_SIZE_EXCEEDED` will be
