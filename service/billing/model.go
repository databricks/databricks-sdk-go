...
type CreateLogDeliveryConfigurationParams struct {
	...
	//     the [View billable usage]
	//   - If `log_type` is `AUDIT_LOGS`, this value must be `JSON`. Only the
	//     JSON (JavaScript Object Notation) format is supported. For the
	...
	// the configuration later. Deletion of a configuration is not supported, so
	// disable a log delivery configuration that is no longer needed.
...
type LogDeliveryStatus struct {
	...
	//
	//   - `CREATED`: There were no log delivery attempts since the config was
	//     created.
	//   - `SUCCEEDED`: The latest attempt of log delivery has succeeded
	//     completely.
	...
	//   - `SYSTEM_FAILURE`: The latest attempt of log delivery failed because
	//     of an Databricks internal error. Contact support if it doesn't go
	//     away soon.
	//   - `NOT_FOUND`: The log delivery status as the configuration has been
	//     disabled since the release of this feature or there are no workspaces
	//     in the account.
