...
// *audit logs* (AWS and GCP). This feature is in Public Preview. This feature
...
//     storage configuration object that uses the bucket name.
...
//     call the Account API to create a credential configuration object that
//     uses the IAM role's ARN.
//  3. **Create log delivery configuration**: Using Databricks APIs, call the
//     Account API to create a log delivery configuration that uses the
...
//     account (*Account level* delivery) or only events for a specific set of
//     workspaces (*workspace level* delivery). Account level log delivery
...
//     delivery]. For the CSV schema, see the [Usage page].
...
//   - All billable usage logs apply to specific workspaces (*workspace level*
//     logs). You can aggregate usage for your entire account by creating an
//     *account level* delivery configuration that delivers logs for all current
//     and future workspaces in your account.
...
//     delivery [AWS] or [GCP], which includes information about the used JSON
//     schema.
//   - The delivery location is
//     `<bucket-name>/<delivery-path-prefix>/workspaceId=<workspaceId>/date=<yyyy-mm-dd>/auditlogs_<internal-id>.json`.
//     Files may get overwritten with the same content multiple times to achieve
//     exactly-once delivery.
//   - If the audit log delivery configuration included specific workspace IDs,
//     only *workspace-level* audit logs for those workspaces are delivered. If
//     the log delivery configuration applies to the entire account (*account
//     level* delivery configuration), the audit log delivery includes
//     workspace-level audit logs for all workspaces in the account as well as
//     account-level audit logs. See Audit log delivery [AWS] or [GCP] for
//     details.
...
type LogDeliveryService interface {
	...
	// you already created a credential object (which encapsulates a
	// cross-account service IAM role) and a storage configuration object (which
	...
	// (see Enable or disable log delivery configuration).
	//
	...
	// described under Create log delivery.
