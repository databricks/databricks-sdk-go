...
type ExperimentsInterface interface {
	...
	//     key. That is, if multiple tag values with the same key are provided in
	//     the same API request, the last-provided tag value is written. Logging the
	//     same tag (key, value) is permitted. Specifically, logging a tag is
	//     idempotent.
