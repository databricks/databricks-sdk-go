...
type ClusterInstance struct {
    ...
    // always available for runs on existing clusters. For runs on new 
    // clusters, it becomes available once the cluster is created. This value 
    // can be used to view logs by browsing to 
    // `/#setting/sparkui/$cluster_id/driver-logs`. The logs continue to be 
    // available after the run completes. The response won?t include this field 
    // if the identifier is not available yet. 
...
type CreateJobRequest struct {
    ...
    // job begin or complete as well as when this job is deleted. The default 
    ...
    // job concurrently. This is useful for example if you trigger your job on 
    // a frequent schedule and want to allow consecutive runs to overlap with 
    // each other, or if you want to trigger multiple runs which differ by 
    // their input parameters. This setting affects only new runs. For example, 
    ...
    // than 3 active runs. This value cannot exceed 1000\. Setting this value 
    // to 0 causes all new runs to be skipped. The default behavior is to allow 
    ...
    // cluster as cluster tags for jobs clusters, and are subject to the same 
...
type GetJobResponse struct {
    ...
    // using the [Reset](..dev-tools/api/latest/jobshtml#operation/JobsReset) 
    // or [Update](..dev-tools/api/latest/jobshtml#operation/JobsUpdate) 
    // endpoints. 
...
type GetRunResponse struct {
    ...
    // any associated artifacts. The total duration of the run is the sum of 
    // the setup_duration, the execution_duration, and the cleanup_duration. 
    ...
    // The canonical identifier of the run. This ID is unique across all runs 
    // of all jobs. 
    RunId int64 `json:"run_id,omitempty"`
    // An optional name for the run. The maximum allowed length is 4096 bytes 
    // in UTF-8 encoding. 
    ...
    // periodic scheduler. 
    Schedule *CronSchedule `json:"schedule,omitempty"`
    // The time it took to set up the cluster in milliseconds. For runs that 
    // run on new clusters this is the cluster creation time, for runs that run 
    // on existing clusters this time should be very short. 
...
type GitSource struct {
    ...
    // cannot be specified in conjunction with git_branch or git_commit. The 
    // maximum length is 255 characters. 
...
type JobTaskSettings struct {
    ...
    EmailNotifications *JobEmailNotifications `json:"email_notifications,omitempty"`
    // If existing_cluster_id, the ID of an existing cluster that is used for 
    // all runs of this task. When running tasks on an existing cluster, you 
    ...
    // parent job. On Update or Reset, this field is used to reference the 
    // tasks to be updated or reset. The maximum length is 100 characters. 
...
type NewCluster struct {
    ...
    // [List node types](..dev-tools/api/latest/clustershtml#list-node-types) 
    ...
    // of 10 workers, whereas the workers listed in `spark_info` gradually 
    ...
    SparkConf []any /* MISSING TYPE */ `json:"spark_conf,omitempty"`
    ...
    SparkEnvVars []any /* MISSING TYPE */ `json:"spark_env_vars,omitempty"`
    // The Spark version of the cluster. A list of available Spark versions can 
    // be retrieved by using the [Runtime 
    // versions](..dev-tools/api/latest/clustershtml#runtime-versions) API 
...
type NotebookOutput struct {
    ...
    // larger result, your job can store the results in a cloud storage 
...
type NotebookTask struct {
    ...
    // parameters specified, the two parameters maps are merged. If the same 
    // key is specified in `base_parameters` and in `run-now`, the value from 
    // `run-now` is used. Use [Task parameter 
    // variables](..jobshtml#parameter-variables) to set parameters containing 
    // information about job runs. If the notebook takes a parameter that is 
    // not specified in the job?s `base_parameters` or the `run-now` override 
    ...
    BaseParameters []any /* MISSING TYPE */ `json:"base_parameters,omitempty"`
    ...
    // must be absolute and begin with a slash. For notebooks stored in a 
...
type RepairRunRequest struct {
    ...
    // cannot be specified in conjunction with notebook_params. The JSON 
    // representation of this field (for example `{&#34;jar_params&#34;:[&#34;john 
    ...
    // exceed 10,000 bytes. 
    NotebookParams []string `json:"notebook_params,omitempty"`
    
    PipelineParams *PipelineParams `json:"pipeline_params,omitempty"`
    // A map from keys to values for jobs with Python wheel task, for example 
    // `&#34;python_named_params&#34;: {&#34;name&#34;: &#34;task&#34;, &#34;data&#34;: 
    // &#34;dbfs:/path/to/data.json&#34;}`. 
    PythonNamedParams []string `json:"python_named_params,omitempty"`
...
// run of the same job, the cluster and execution context are being prepared. 
// If there is already an active run of the same job, the run immediately 
...
type RunNowRequest struct {
    ...
    // run with the provided token already exists, the request does not create 
    // a new run but returns the ID of the existing run instead. If a run with 
    // the provided token is deleted, an error is returned. If you specify the 
    // idempotency token, upon failure you can retry until the request 
    // succeeds. Databricks guarantees that exactly one run is launched with 
    // that idempotency token. This token must have at most 64 characters. For 
...
type SubmitRunRequest struct {
    ...
    // run is launched with that idempotency token. This token must have at 
    // most 64 characters. For more information, see [How to ensure idempotency 
