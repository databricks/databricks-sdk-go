...
type GetRunRequest struct {
	...
	IncludeHistory bool `json:"-" tf:"-"`
	// Whether to include resolved parameter values in the response.
	IncludeResolvedValues bool `json:"-" tf:"-"`
...
type ModelTriggerConfiguration struct {
	...
	// The level determines the scope of the trigger. It can be set to
	// metastore, schema, or model level.
	// Wire name: 'level'
...
func (st *ModelTriggerConfiguration) UnmarshalJSON(b []byte) error {
	if st == nil {
		return fmt.Errorf("json.Unmarshal on nil pointer")
	}
	...
	tmp, err := modelTriggerConfigurationFromPb(pb)
	if err != nil {
		return err
	}
	*st = *tmp
...
func (st ModelTriggerConfiguration) MarshalJSON() ([]byte, error) {
	pb, err := modelTriggerConfigurationToPb(&st)
	if err != nil {
		return nil, err
	}
	return json.Marshal(pb)
...
type ModelTriggerConfigurationLevel string

const ModelTriggerConfigurationLevelMetastore ModelTriggerConfigurationLevel = `METASTORE`

const ModelTriggerConfigurationLevelModel ModelTriggerConfigurationLevel = `MODEL`

const ModelTriggerConfigurationLevelSchema ModelTriggerConfigurationLevel = `SCHEMA`

// String representation for [fmt.Print]
func (f *ModelTriggerConfigurationLevel) String() string {
	return string(*f)
}

// Set raw string value and validate it against allowed values
func (f *ModelTriggerConfigurationLevel) Set(v string) error {
	switch v {
	case `METASTORE`, `MODEL`, `SCHEMA`:
		*f = ModelTriggerConfigurationLevel(v)
		return nil
	default:
		return fmt.Errorf(`value "%s" is not one of "METASTORE", "MODEL", "SCHEMA"`, v)
	}
}

// Values returns all possible values for ModelTriggerConfigurationLevel.
//
// There is no guarantee on the order of the values in the slice.
func (f *ModelTriggerConfigurationLevel) Values() []ModelTriggerConfigurationLevel {
	return []ModelTriggerConfigurationLevel{
		ModelTriggerConfigurationLevelMetastore,
		ModelTriggerConfigurationLevelModel,
		ModelTriggerConfigurationLevelSchema,
	}
}

// Type always returns ModelTriggerConfigurationLevel to satisfy [pflag.Value] interface
func (f *ModelTriggerConfigurationLevel) Type() string {
	return "ModelTriggerConfigurationLevel"
}
