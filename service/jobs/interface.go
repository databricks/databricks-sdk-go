...
type JobsService interface {
	...
	CancelRun(ctx context.Context, cancelRun CancelRun) error
	// CancelRun and wait to reach TERMINATED or SKIPPED state
	CancelRunAndWait(ctx context.Context, request CancelRun, timeout ...time.Duration) (*Run, error)
	CancelRunByRunId(ctx context.Context, runId int64) error
	CancelRunByRunIdAndWait(ctx context.Context, runId int64, timeout ...time.Duration) (*Run, error)
	// Create a new job.
	...
	GetRun(ctx context.Context, getRunRequest GetRunRequest) (*Run, error)
	...
	RepairRun(ctx context.Context, repairRun RepairRun) (*RepairRunResponse, error)
	// RepairRun and wait to reach TERMINATED or SKIPPED state
	RepairRunAndWait(ctx context.Context, request RepairRun, timeout ...time.Duration) (*Run, error)
	...
	RunNowAndWait(ctx context.Context, request RunNow, timeout ...time.Duration) (*Run, error)
	...
	SubmitAndWait(ctx context.Context, request SubmitRun, timeout ...time.Duration) (*Run, error)
