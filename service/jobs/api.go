...
type JobsService interface {
    ...
    // that the run_id parameter is valid and returns an HTTP status code 400 
    // if the run_id parameter is invalid. Runs are automatically removed after 
    // 60 days. If you to want to reference them beyond 60 days, you must save 
    // old run results before they expire. To export using the UI, see Export 
...
// 5 MB of the output. To return a larger result, you can store job results in 
// a cloud storage service. This endpoint validates that the run_id parameter 
// is valid and returns an HTTP status code 400 if the run_id parameter is 
